...
const getGQL = (urlPath, query, variables) => { 
...
// Get messeages from graphQL later then last message timeStamp (lastMessageID)
async function getMessages(lastMessageID) {

    // GraphiQL request string to get all categories
    let query = `query GetMessages($timeStamp:String){
        getMessages(timeStamp:$timeStamp) {
            nick,
            message,
            timeStamp,
        }
    ...
    return await getGQL(URL + "/graphql", query, {timeStamp: lastMessageID} )
        .then(res => res.data.getMessages )
        .then(data => {
            console.log(data)
            return data
        })
...
// Send message to graphGL
async function sendMessage(nick, message) {

    // GraphiQL request string to get all categories
    let query = `mutation SendMessage($nick:String, $message:String){
        addMessage(message:{nick:$nick, message:$message}){
            nick, 
            message,
            timeStamp,
        }
    ...
    await getGQL(URL + "/graphql", query, {nick: nick, message: message} )
        .then(res => res.data)
...
const actionGet = () =>
    async dispatch => {
        try {
            ...
            // Get messeages later then last message timeStamp (lastMessageID)
            let result = await getMessages(lastMessageID) 
