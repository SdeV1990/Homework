...
// In this chat there is no need to use version key
const messageScheme = new Schema({ nick: String, message: String }, {versionKey: false})
const Message = mongoose.model("Message", messageScheme)

// Connection to mongoose
const PORT = 4000
mongoose.connect("mongodb://localhost:27017/messageDB", { useUnifiedTopology: true, useNewUrlParser: true, useFindAndModify: false }, function(error){
...
app.post("/message", jsonParser, (request, response) => {
    ...
    request.on("end", () => {
        try{
            ...
            response.send("WRONG JSON")
        }
    })
})

// Get messages
app.post("/message-after", (request, response) => {
    ...
    request.on("end", () => {
        try{
            ...
            Message.find({"_id": { $gt: constructedObjectId }}, function(error, messages){
                 
                // Return error
                if(error) return console.log("Problem with getting messages from data base. " + error)
