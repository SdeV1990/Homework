...
// Добавьте условие отрицательного возраста в пример выше. Расставьте недостающие (но синтаксически необязательные) фигурные скобки. Выкиньте лишнее из текущего кода
    ...
    if (isNaN(age)) alert("ОШИБКА: введено нечисловое значение! Введите число.")
    else if (age <= 0) alert("Всё спланировали?")
    else if (age < 18) alert("Школьник.")
    else if (age < 30) alert("Молодежь.")
    else if (age < 45) alert("Зрелость.")
    else if (age < 60) alert("Закат.")
    else if (age < 124) alert("Как пенсия?")
    else alert("Киборг.")
