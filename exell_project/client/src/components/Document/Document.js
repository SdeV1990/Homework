...
const Document = ( { document, getDocument, actionCalculateCellsValue }) => {
    ...
    let rowHeight = (!!document.document && !!document.document.sheets[0]) 
        ? document.document.sheets[0].rowHeight 
        : {}
                ...
                <td 
                    ...
                    id={`${cellID}_cell`}>
                        {/* <textarea
                            ...
                            // onBlur={ (event) => handleCellBlur(event) }
    ...
    // console.log(rowHeight[+convertAddressToCoorginates("A1").row])
